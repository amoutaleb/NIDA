...
async def ingest_fires_impl(db: Session) -> IngestResult:
    ...
    # Persistence filter: suppress detections in grid cells that have
    # lit up across many distinct days (industrial/static heat sources
    # such as cement plants, power stations, and refineries, which the
    # land-cover filter misses in the populated north). Self-learning
    # from detection history; see geo/persistence.py.
    from backend.geo.persistence import filter_static_sources
    df, static_dropped = filter_static_sources(db, df)

    # Known industrial-source filter: suppress detections sitting on
    # OSM-mapped industrial facilities (power plants, cement works,
    # refineries, industrial zones). Works from the first run without any
    # accumulated history, complementing the self-learning filter above.
    from backend.geo.industrial import filter_industrial_sources
    df, industrial_dropped = filter_industrial_sources(df)
