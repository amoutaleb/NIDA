...
class RouteResult:
    ...
    avoids_fires: bool = True      # False if this is the no-avoidance fallback route
...
# OpenRouteService avoid-polygon limits (free tier): each avoid area must
# not exceed ~20 km in width/height, the total avoid area is bounded, and
# routing with avoid areas is capped at 150 km. Our wind-driven ellipses
# can be much longer than 20 km downwind, which makes ORS reject the whole
# request -- the observed "no route anywhere" failure. We therefore send
# ORS a SIZE-CAPPED proxy polygon per zone (a small box around the fire
# core) rather than the full display ellipse. The full ellipse is still
# shown to the user; only the routing-avoidance geometry is capped.
_ORS_MAX_AVOID_HALF_DEG = 0.09   # ~10 km half-extent -> ~20 km box, ORS's per-polygon limit


def _capped_avoid_polygon(cluster: FireClusterModel) -> list:
    """
    Build a small, ORS-safe avoid polygon around a fire's core: a box
    centred on the cluster centroid, clamped to ORS's per-polygon extent
    limit. Keeps the router away from the fire itself without exceeding
    the avoid-area size ORS will accept.
    """
    half_km = min(cluster.semi_minor_km or 4.0, 10.0)
    half_deg = min(half_km / 111.0, _ORS_MAX_AVOID_HALF_DEG)
...
async def _post_route(body: dict) -> Optional[dict]:
    """POST to ORS; return parsed JSON on success, None on any failure."""
    headers = {"Authorization": settings.ORS_API_KEY, "Content-Type": "application/json"}
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            resp = await client.post(settings.ORS_BASE_URL, json=body, headers=headers)
    except httpx.HTTPError as exc:
        logger.warning(f"ORS unreachable: {exc}")
        return None
    if resp.status_code != 200:
        logger.warning(f"ORS HTTP {resp.status_code}: {resp.text[:200]}")
        return None
    data = resp.json()
...
async def get_route(
    ...
    OpenRouteService, avoiding the given fire clusters.

    Reliability-first design (fixes the observed all-routes-fail case):
      1. Fire zones are sent as SIZE-CAPPED avoid boxes so the request
         stays within ORS's avoid-area limits (full ellipses often exceed
         them and make ORS reject everything).
      2. If routing WITH avoidance still fails, we retry WITHOUT avoidance
         and return that route flagged avoids_fires=False, so the user
         gets a real escape path with a clear warning rather than a
         dead-end "no route". Honest degradation beats nothing.

    Raises EvacuationError only if even the unrestricted route fails
    (genuinely no road connection / service down).
    """
    ...
    if avoid_clusters:
        ...
        body["options"] = {
            "avoid_polygons": {
                "type": "MultiPolygon",
                "coordinates": [_capped_avoid_polygon(c) for c in avoid_clusters],
            }
        }
        data = await _post_route(body)

    if data is None:
        data = await _post_route(base_body)
        avoids_fires = False

    if data is None:
        raise EvacuationError("No route could be found, even without fire avoidance.")
    ...
    return RouteResult(
        ...
        avoided_zone_ids=[c.id for c in avoid_clusters] if avoids_fires else [],
        avoids_fires=avoids_fires,
    )
