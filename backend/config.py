...
class Settings(BaseSettings):
    ...
    # Static-source (industrial heat) suppression. A cell is classified as
    # industrial only if it is persistent AND low-intensity AND spatially
    # isolated -- see geo/persistence.py. Requiring all three keeps recall
    # high: a large or spreading wildfire can never satisfy them.
    # NOTE: MIN_DISTINCT_DAYS must not exceed FIRMS_DAY_RANGE, or the test
    # can never be satisfied on a fresh database.
    PERSISTENCE_FILTER_ENABLED: bool = True
    PERSISTENCE_MIN_DISTINCT_DAYS: int = 3   # distinct days a cell must recur on
    PERSISTENCE_LOOKBACK_DAYS: int = 60      # history window considered
    STATIC_MAX_FRP_MW: float = 100.0         # industrial sources radiate modestly
    STATIC_MAX_NEIGHBOUR_CELLS: int = 4      # of 8; a spreading fire lights more

    # Known industrial-source filter — suppress detections on OSM-mapped
    # power plants, cement works, refineries, and industrial zones. Works
    # from the first run (no history needed). Buffer is facility-scale so
    # real fires merely near industry are not discarded.
    INDUSTRIAL_FILTER_ENABLED: bool = True
    INDUSTRIAL_FILTER_BUFFER_KM: float = 3.0
